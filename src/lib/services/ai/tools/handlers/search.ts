...
export async function handleSearchSymbols(args: Record<string, unknown>): Promise<ToolResult> {
  ...
  try {
    // Use workspace symbol search from LSP
    const symbols = await invoke<Array<{
      name: string;
      kind: number;
      location: {
        uri: string;
    ...
    const fallbackResult = await handleWorkspaceSearch({
      query: `(function|class|const|let|var|interface|type|enum)\\s+${query}\\b`,
      includePattern: '**/*.{ts,tsx,js,jsx,svelte,vue}',
      caseSensitive: false
    });

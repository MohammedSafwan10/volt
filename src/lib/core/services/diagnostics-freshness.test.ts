...
describe('diagnostics-freshness', () => {
  ...
  it('classifies fresh and stale sources by last update age', () => {
    ...
    expect(getDiagnosticSourceHealth(10_000, false, false, 10_000 + DIAGNOSTICS_FRESH_MS + 1)).toBe('stale');
  ...
  it('summarizes mixed source states', () => {
    const summary = summarizeDiagnosticSources(
      [
        ...
        { source: 'eslint', lastUpdated: 10_000, isUpdating: false, isStale: false, fileCount: 1, problemCount: 1 },
    ...
    expect(summary.sourceStatuses.map((item) => item.status)).toEqual(['fresh', 'stale', 'updating']);
  });
});

...
describe('projectStore workspace lifecycle', () => {
    ...
    invokeMock.mockImplementation(async (command: string, args?: any) => {
      switch (command) {
        ...
        case 'workspace_open':
          return {
            opened: true,
            activeRootPath: args.request.path,
            previousRootPath: args.request.currentRootPath ?? null,
            unchanged: false,
            recentProjects: [args.request.path],
            message: null,
          };
        ...
        case 'workspace_refresh':
          return {
            refreshed: true,
            activeRootPath: args.request.currentRootPath ?? null,
            recentProjects: ['C:/workspace'],
            message: null,
          };
        case 'workspace_replace_recent_projects':
          return args.recentProjects;
        default:
          return undefined;
      }
    });
  ...
  it('preserves current workspace and shows feedback when opening an invalid folder fails', async () => {
    ...
    invokeMock.mockImplementationOnce(async (command: string, args?: any) => {

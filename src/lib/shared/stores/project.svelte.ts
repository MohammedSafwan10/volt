...
import {
  ...
  type WorkspaceActivationPlan,
  type WorkspaceActivationTask,
  type WorkspaceActivationTaskKind,
} from "$core/workspace/workspace-lifecycle";
import {
  cleanupEditorStore,
  cleanupMcpStore,
...
class ProjectStore {
  ...
  private registerWorkspaceLifecycleHooks(): void {
    this.workspaceLifecycle.register({
      ...
      activate: async ({ rootPath, reuseExistingWorkspace = false }) => {
        ...
        const initialActivationPlan = await this.workspaceLifecycle.planActivation({
          rootPath,
        ...
        this.startupSerialChain = Promise.resolve();
        this.scheduleActivationPlanTasks(
          rootPath,
          generation,
          this.resolveActivationTasks(initialActivationPlan),
          reuseExistingWorkspace,
        );
      },
    });
  ...
  private resolveActivationTasks(plan: WorkspaceActivationPlan): WorkspaceActivationTask[] {
    return plan.tasks;
  }

  private scheduleActivationPlanTasks(
    rootPath: string,
    generation: number,
    ...
    for (const task of tasks) {
      const runTask = async () => {
        await this.executeActivationTask(rootPath, generation, task, reuseExistingWorkspace);
      };
      if (task.serial) {
        this.scheduleSerialActivationTask(rootPath, generation, task.phase, task.delayMs, runTask);
      } else {
        this.scheduleActivationTask(rootPath, generation, task.delayMs, runTask);
      }
    }
  }

  private async executeActivationTask(
    rootPath: string,
    generation: number,
    ...
    switch (task.kind) {
      ...
      case "index_project": {
        ...
        const indexedActivationPlan = await this.workspaceLifecycle.planActivation({
          rootPath,
          reuseExistingWorkspace,
          indexedCount: status.count,
          initialIndexDurationMs: indexDurationMs,
        });
        if (!this.isStartupCurrent(rootPath, generation)) return;

        this.indexedFileCount = status.count;
        this.initialIndexDurationMs = indexDurationMs;
        this.largeRepoMode = indexedActivationPlan.largeRepoMode;
        this.coreReady = true;

        if (indexedActivationPlan.tasks.length === 0) {
        ...
        this.scheduleActivationPlanTasks(
          rootPath,
          generation,
          this.resolveActivationTasks(indexedActivationPlan),
          reuseExistingWorkspace,
        );
        return;
      }
      ...
      case "initialize_mcp":
        ...
        return;
      case "warm_semantic_index":
        await warmSemanticIndex(rootPath);
        return;
      case "finalize_background":
        this.backgroundReady = true;
        return;
      default: {
        const exhaustiveCheck: never = task.kind;
  ...
  private scheduleActivationTask(
    ...
    void (async () => {
      await this.workspaceLifecycle.waitForActivationDelay(delayMs);
      if (!this.isStartupCurrent(rootPath, generation)) return;
      await task();
    })().catch((error) => {
  ...
  private scheduleSerialActivationTask(
    ...
    void (async () => {
      await this.workspaceLifecycle.waitForActivationDelay(delayMs);
      if (!this.isStartupCurrent(rootPath, generation)) return;
      this.startupSerialChain = this.startupSerialChain
        .then(async () => {
          if (!this.isStartupCurrent(rootPath, generation)) return;
  ...
  private async handleFileChanges(batch: FileChangeBatchEvent): Promise<void> {
    ...
    const shouldRefreshDiagnostics = this.rootPath && this.shouldTriggerProjectDiagnostics(batch);
  ...
  async expandFolder(node: TreeNode, maxDepth = 2): Promise<void> {
    ...
    const expand = async (current: TreeNode, depth: number): Promise<void> => {
      ...
      if (current.children === null && depth < maxDepth) {
        ...
        if (entries !== null) {
          current.children = this.sortEntries(entries).map((entry) => this.createTreeNode(entry));
        }
      }
